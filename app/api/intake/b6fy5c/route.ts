...
// (control) condition doesn't ask those questions. `english_proficiency` and
// `essay_writing_frequency` are required for every condition.
const Body = z.object({
  ...
  english_proficiency: z.number().int().min(1).max(5),
